...
class BinaryTree:
    ...
    def find_r(self, target):
        if self.root:                                           #If "self.root" is not "None" (i.e. Exists), exexute the "if" code block
            if self._find_r(target, self.root):                 #If "._find_r" sub-method returns "True" after recursively searching instance of object class that called method
                                                                #for "target" starting from the "self.root" (Root Node)

                return True                                     #Return "True" to caller
            return False                                        #Otherwise return "False" to caller
        else:                                                   #Else "self.root" is "None" (i.e. Dosen't exist)
            return None                                         #Return "None" to caller
...
print("Is value '8' present in \"bst\" object insance of \"Binary Tree\": " + str(bst.find_i(8)) + '\n')
                                                                #and not present in "bst" instance of "BinaryTree" object class and print the return value; "True", "False" or "None"
